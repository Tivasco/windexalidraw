 ...
 * @param {'info' | 'success' | 'warning' | 'error'} props.variant - Toast variant
 ...
 * @param {number} props.autoHideDuration - Duration in milliseconds before the toast auto-hides (0 to disable)
...
const Toast = ({
  ...
  const { container, icon } = variantStyles[variant] || variantStyles.info;

  return (
    <div 
      className={`fixed ${positionStyles} m-4 transition-all duration-300 transform ${
        isShowing ? 'translate-y-0 opacity-100' : 'translate-y-2 opacity-0'
      }`}
      onAnimationEnd={handleAnimationEnd}
      role="alert"
    >
      <div className={`border rounded-md p-4 shadow-lg max-w-sm ${container} ${className}`}>
        <div className="flex">
